...
class ConfigService {
  ...
  bool get isMaintenanceMode =>
      getAppSetting<bool>('maintenance_mode', defaultValue: false) ?? false;

  /// Obtener email de soporte
  String get supportEmail =>
      getAppSetting<String>(
        'support_email',
        defaultValue: 'soporte@ayutthayacamp.com',
      ) ??
      'soporte@ayutthayacamp.com';

  /// Obtener teléfono de soporte
  String get supportPhone =>
      getAppSetting<String>('support_phone', defaultValue: '+506-1234-5678') ??
      '+506-1234-5678';

  /// Obtener capacidad por defecto de las clases
  int get defaultClassCapacity =>
      getAppSetting<int>('default_class_capacity', defaultValue: 30) ?? 30;

  /// Obtener días máximos de reserva anticipada
  int get maxAdvanceBookingDays =>
      getAppSetting<int>('max_advance_booking_days', defaultValue: 7) ?? 7;

  /// Obtener precio de matrícula
  double get enrollmentPrice {
    final price = getPaymentSetting<num>(
      'enrollment_price',
      defaultValue: 30000,
    );
  ...
  String get currencySymbol =>
      getPaymentSetting<String>('currency_symbol', defaultValue: '₡') ?? '₡';

  /// Obtener moneda
  String get currency =>
      getPaymentSetting<String>('currency', defaultValue: 'CRC') ?? 'CRC';

  /// Obtener métodos de pago disponibles
  List<String> get paymentMethods {
    final methods = getPaymentSetting<List<dynamic>>(
      'payment_methods',
      defaultValue: ['sinpe', 'transferencia', 'efectivo'],
    );
    return methods?.map((e) => e.toString()).toList() ??
        ['sinpe', 'transferencia', 'efectivo'];
  }

  /// Verificar si se requiere comprobante
  bool get requireReceipt =>
      getPaymentSetting<bool>('require_receipt', defaultValue: true) ?? true;

  /// Verificar si la aprobación automática está habilitada
  bool get autoApproveEnabled =>
      getPaymentSetting<bool>('auto_approve_enabled', defaultValue: false) ??
      false;

  /// Verificar si los pagos están habilitados
  bool get paymentsEnabled =>
      isFeatureEnabled('payments_enabled', defaultValue: true);

  /// Verificar si las reservas están habilitadas
  bool get bookingEnabled =>
      isFeatureEnabled('booking_enabled', defaultValue: true);

  /// Verificar si el check-in con QR está habilitado
  bool get qrCheckinEnabled =>
      isFeatureEnabled('qr_checkin_enabled', defaultValue: true);

  /// Verificar si el chat de soporte está habilitado
  bool get chatSupportEnabled =>
      isFeatureEnabled('chat_support_enabled', defaultValue: false);

  /// Verificar si las notificaciones push están habilitadas
  bool get pushNotificationsEnabled =>
      isFeatureEnabled('push_notifications_enabled', defaultValue: true);

  /// Verificar si los reportes de admin están habilitados
  bool get adminReportsEnabled =>
      isFeatureEnabled('admin_reports_enabled', defaultValue: true);

  /// Obtener nombre del gimnasio
  String get gymName =>
      getBusinessInfo<String>('gym_name', defaultValue: 'Ayutthaya Camp') ??
      'Ayutthaya Camp';
  ...
  String get schedule =>
      getBusinessInfo<String>(
        'schedule',
        defaultValue: 'Lun-Vie: 7am-10pm, Sáb: 9am-2pm',
      ) ??
      'Lun-Vie: 7am-10pm, Sáb: 9am-2pm';

  /// Obtener descripción del negocio
  String get about =>
      getBusinessInfo<String>(
        'about',
        defaultValue: 'Gimnasio especializado en Muay Thai y Boxing',
      ) ??
      'Gimnasio especializado en Muay Thai y Boxing';

  /// Obtener redes sociales
  Map<String, dynamic> get socialMedia {
    final social = getBusinessInfo<Map<String, dynamic>>(
      'social_media',
      defaultValue: {},
    );
    return social ??
